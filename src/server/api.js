...
router.get('/all', async (req, res) => {
  ...
  if (req.query.groupBy === 'totals') {
    const c = {
      label: 'COVID-19 Cases', backgroundColor: colors[2], borderColor: colors[2], yAxisID: 'covid', tension: 0, fill: true, data: dbResults.map((e) => ({ x: e.date, y: e.covid_male + e.covid_female })),
    };
    const i = {
